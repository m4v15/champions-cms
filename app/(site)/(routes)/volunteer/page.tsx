...
export default async function Volunteer() {
  ...
  const components: PortableTextComponents = {
    block: {
      h2: ({ children }) => (
        <h2 className="mb-3 text-2xl font-bold text-gray-800 md:text-3xl lg:text-4xl lg:leading-tight xl:text-5xl xl:leading-tight">
          {children}
        </h2>
      ),
      normal: ({ children }) => (
        <p className="ms-1 mt-5 text-gray-800 sm:mb-1 sm:ms-2">{children}</p>
      ),
    },
  };
